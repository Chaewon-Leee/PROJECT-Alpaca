...
def generate_instruction_following_data(
    ...
    request_batch_size=5, # batch_size 횟수
			### num_prompt_instructions개씩 request_batch_size번
				### = 총 15개의 랜덤 샘플을 가져옴
    temperature=1.0,
    top_p=1.0,
    num_cpus=16,
    ...
    # similarities = {}
		### 유사도 체크용 : 너무 유사한 문장은 제외
    scorer = rouge_scorer.RougeScorer(["rougeL"], use_stemmer=False)

    # now let's generate new instructions!
		### num_instructions_to_generate : 몇개의 문장을 생성
    progress_bar = tqdm.tqdm(total=num_instructions_to_generate)
    if machine_instruction_data:
        progress_bar.update(len(machine_instruction_data))

    # first we tokenize all the seed instructions and generated machine instructions
		### all_instructions = seed_instruction_data (기존 seed로 지니고 있던 데이터셋) + machine_instruction_data (모델로 생성한 데이터셋) 결합하여 사용
    all_instructions = [d["instruction"] for d in seed_instruction_data] + [
        d["instruction"] for d in machine_instruction_data
    ]
    all_instruction_tokens = [scorer._tokenizer.tokenize(inst) for inst in all_instructions]

		### 100개의 instruction을 반복적으로 생성
    while len(machine_instruction_data) < num_instructions_to_generate:
        ...
        for _ in range(request_batch_size):
            # only sampling from the seed tasks
						### seed_instruction_data에서 num_prompt_instructions 개수 만큼 랜덤으로 가져와
        ...
        request_start = time.time()
				### openAI에서 해당 기존 seed에서 랜덤으로 가져온 instruction으로 생성
        ...
        for instruction_data_entry in instruction_data:
            ...
            with Pool(num_cpus) as p:
								### 생성된 문장과 기존 문장들을 rouge_scores을 이용해 유사도 측정
                rouge_scores = p.map(
...
if __name__ == "__main__":
    fire.Fire(main) ### command line interface로 변경
